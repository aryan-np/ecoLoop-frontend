...
export default function SavedDonationRequests() {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const data = await donationAPI.getNGOSavedRequests();
      ...
      console.error('Error loading saved donation requests:', error);
      setRequests([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleToggleSave = async (requestId, isSaved) => {
    try {
      ...
      if (isSaved) {
        await donationAPI.unsaveNGOPendingRequest(requestId);
        setRequests((prev) => prev.filter((request) => request.id !== requestId));
      } else {
        await donationAPI.saveNGOPendingRequest(requestId);
  ...
  return (
    <div className="bg-gray-50 p-6 min-h-full">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900">Saved Donation Requests</h1>
        <p className="text-gray-600 mt-2">Review the donation requests you have saved</p>
      </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-500 text-lg">No saved donation requests</p>
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-gray-200">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className={`px-3 py-2 text-sm font-semibold rounded-lg transition border text-purple-700 bg-purple-100 border-purple-200 hover:bg-purple-200 ${
